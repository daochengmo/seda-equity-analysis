...
This report applies descriptive data science methods to the Stanford Education Data Archive (SEDA, v2024.3) to characterize the geographic and demographic structure of academic achievement gaps in U.S. public school districts in 2024, the most recent year available. Across roughly 11,000 districts, the median White-Black math gap is 0.65 standard deviations (SD) — equivalent to roughly two grade levels of learning — and the median White-Hispanic and non-ECD vs. ECD gaps are 0.42 SD and 0.51 SD, respectively. Gaps are concentrated in the Upper Midwest, with Wisconsin (median 1.15 SD) showing the largest within-state district gaps among states with sufficient computable district estimates. Counter to a common assumption, district-level White-Black gaps are *negatively* associated with district poverty (r = -0.24): gaps are widest in low-poverty, demographically mixed districts, a pattern often associated with segregation-based accounts of inequality (Reardon & Owens, 2014). Within-district comparison of 2019 and 2024 shows that 62-65% of districts saw racial gaps *widen* during the pandemic period, with White-Hispanic gaps growing fastest (median +0.06 SD). This analysis was conducted in R using SEDA's public covariate and achievement files, and code is available at the GitHub repository linked in the appendix. Because the analysis is descriptive, these patterns should be read as evidence of geographic and demographic structure rather than causal estimates.
...
Taken together, these mechanisms suggest that the negative association in Figure 4 should not be read as evidence that poverty is unimportant. Rather, it indicates that district-level poverty and within-district racial gap size capture different dimensions of inequality. High-poverty districts may have low average achievement for all groups, while lower-poverty or middle-income districts may contain sharper internal stratification by race. For district leaders, this distinction matters: an equity strategy focused only on the highest-poverty districts may miss districts where racial achievement gaps are especially wide. Future analysis should test this interpretation more directly by incorporating within-district segregation measures, school-level composition, course-taking patterns, and district size.
...
The second is that the direction matters more than the magnitude in a study of this scale. If pandemic disruption affected districts randomly with respect to equity — some districts' gaps narrowed, some widened, with the overall distribution centered on zero — we would expect roughly 50% of districts to show widening. The observation that 62-65% of districts moved in the same direction suggests a systematic descriptive pattern rather than a distribution centered evenly around zero. It suggests a systematic descriptive tilt in which the typical U.S. district moved modestly away from gap reduction during the pandemic period. The shift is small per district but consistent in sign across thousands of districts, which is the pattern that should concern district leaders and state policymakers.
The contrast between racial/ethnic gaps and the non-ECD vs. ECD gap should be interpreted cautiously. One possible explanation is that pandemic-era recovery efforts organized around income may have aligned more directly with socioeconomic disadvantage than with racial and ethnic stratification. Another possibility is that racial/ethnic gaps capture dimensions of school segregation, language access, and instructional opportunity that are not fully represented by economic disadvantage alone. Distinguishing among these explanations would require additional analysis of funding allocation, school-level composition, and district recovery policies.
